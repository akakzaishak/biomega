...
<style>
    ...
    .dp-card.selected {
        ...
        box-shadow: 0 14px 34px rgba(0, 94, 164, .12);
    }
    #map {
        width: 100%;
        height: clamp(500px, calc(100vh - 240px), 820px);
        min-height: 500px;
            ...
            @else
                <div class="flex-1 overflow-y-auto divide-y divide-outline-variant/10 max-h-[calc(100vh-300px)] px-2 py-2">
                    @foreach ($deliveryPersons as $dp)
                        ...
                        @endphp
                            <div class="dp-card p-4 border border-transparent rounded-2xl mx-2 my-2 {{ $isForced && !$isOnline ? 'bg-red-50/60' : 'bg-white' }}"
                             data-phone="{{ $phone }}"
                            ...
                            <div class="flex items-start gap-3">
                                <div class="relative flex-shrink-0">
                                    <div class="w-11 h-11 rounded-2xl flex items-center justify-center text-white font-bold text-sm shadow-sm" style="background:{{ $color }}">
                            ...
                            @if (!$isOnline)
                                <form method="POST" action="{{ route('admin.tracking') }}" onsubmit="return confirmForce('{{ $dp['FirstName'].' '.$dp['LastName'] }}')" onclick="event.stopPropagation()">
                                    ...
                                    <button type="submit"
                                        class="mt-4 w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-2xl text-xs font-black uppercase tracking-wide transition-all active:scale-95 {{ $isForced ? 'bg-orange-100 text-orange-600 border border-orange-300 hover:bg-orange-200' : 'bg-error text-white hover:bg-red-700 force-btn-pulse shadow-[0_8px_24px_rgba(186,26,26,.22)]' }}">
