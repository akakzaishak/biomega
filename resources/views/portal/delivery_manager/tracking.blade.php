    ...
    <div class="fade-in flex items-center justify-between flex-wrap gap-3">
      ...
      <div class="flex gap-3 flex-wrap">
        <div class="bg-surface-container-lowest border border-outline-variant/15 rounded-xl px-4 py-2 flex items-center gap-2">
          ...
          <span class="text-sm font-bold text-tertiary">{{ $onlineDP }} Online</span>
        </div>
        <div class="bg-surface-container-lowest border border-outline-variant/15 rounded-xl px-4 py-2 flex items-center gap-2">
          <span class="w-2.5 h-2.5 rounded-full bg-slate-400"></span>
          <span class="text-sm font-bold text-slate-500">{{ $offlineDP }} Offline</span>
        </div>
        @if ($forcedCount > 0)
        <div class="bg-error/10 border border-error/20 rounded-xl px-4 py-2 flex items-center gap-2">
          ...
          <span class="text-sm font-bold text-error">{{ $forcedCount }} Awaiting GPS</span>
        </div>
        @endif
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-[360px_1fr] gap-5 fade-in">
      <div class="bg-surface-container-lowest rounded-2xl border border-outline-variant/15 shadow-sm flex flex-col overflow-hidden">
        ...
        <div class="overflow-y-auto flex-1 divide-y divide-outline-variant/10">
          ...
          @endphp
          <div class="dp-card p-4 border border-transparent rounded-xl mx-2 my-1" data-phone="{{ $phone }}" data-lat="{{ $dp['Latitude'] ?? '' }}" data-lng="{{ $dp['Longitude'] ?? '' }}" onclick="focusDriver(this)">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0" style="background:{{ $color }}">{{ $initials }}</div>
              <div class="flex-1 min-w-0">
                <p class="font-bold text-on-surface text-sm truncate">{{ trim((string) ($dp['FirstName'] ?? '') . ' ' . (string) ($dp['LastName'] ?? '')) }}</p>
            ...
            <div class="mt-2 space-y-1">
              @foreach (array_slice($orders, 0, 2) as $ord)
              <div class="flex items-center justify-between text-[10px] font-medium bg-surface-container-low rounded px-2 py-1">
                <span class="flex items-center gap-1">@if(!empty($ord['IsUrgen']))<span class="material-symbols-outlined text-error text-[11px]">priority_high</span>@endif #{{ $ord['order_id'] }}</span>
                <span class="text-on-surface-variant truncate max-w-[110px]">{{ trim((string) ($ord['ph_first'] ?? '') . ' ' . (string) ($ord['ph_last'] ?? '')) }}</span>
              </div>
              @endforeach
              @if(count($orders) > 2)
            ...
            @if (!$isOnline)
            <form method="POST" action="{{ route('delivery-manager.tracking') }}" onsubmit="return confirmForce('{{ trim((string) ($dp['FirstName'] ?? '') . ' ' . (string) ($dp['LastName'] ?? '')) }}')" onclick="event.stopPropagation()">
              @csrf
              <input type="hidden" name="force_gps_phone" value="{{ $phone }}"/>
              <button type="submit" class="mt-3 w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs font-black uppercase tracking-wide transition-all active:scale-95 {{ $isForced ? 'bg-orange-100 text-orange-600 border border-orange-300 hover:bg-orange-200' : 'bg-error text-white hover:bg-red-700 force-btn-pulse' }}">
                <span class="material-symbols-outlined text-sm" style="font-variation-settings:'FILL' 1;">gps_fixed</span>
...
const TRACKING_FORCE_URL = @json(route('delivery-manager.tracking'));
