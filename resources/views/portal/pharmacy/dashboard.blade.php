  ...
  <main class="flex-1 lg:ml-64 p-4 lg:p-8 space-y-8 bg-surface">
    ...
    <div id="section-create" class="section-panel hidden space-y-6">
      <div class="fade-in">
        ...
        <p class="text-on-surface-variant font-medium mt-1">Add medicines, quantities and order details.</p>
      </div>
      ...
      <div class="fade-in fade-in-1 bg-surface-container-lowest rounded-2xl border border-outline-variant/15 shadow-sm p-6 lg:p-8">
        <form method="POST" action="{{ route('pharmacy.dashboard') }}" id="orderForm" class="space-y-8" onsubmit="return prepareOrderForm();">
          ...
          <input type="hidden" name="pharmacy_nif" value="{{ $pharmacyNifSafe ?? '' }}">
          ...
          <div>
            ...
            <div class="relative mb-3">
              ...
              <input type="text" id="itemSearch" placeholder="Search a medicine..." oninput="filterItems(this.value)" class="w-full pl-10 pr-4 py-3 bg-surface-container-high border-none rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:outline-none text-on-surface" />
            </div>
            ...
            <div id="itemList" class="max-h-52 overflow-y-auto rounded-xl border border-outline-variant/20 divide-y divide-outline-variant/10 mb-4">
              @if(empty($medicineSuggestions))
                <div class="px-4 py-6 text-center text-sm text-on-surface-variant">No medicines in database.</div>
              @else
                @foreach($medicineSuggestions as $item)
                <div class="item-db-row flex items-center justify-between px-4 py-3 hover:bg-primary/5 cursor-pointer transition-colors" data-name="{{ strtolower($item['name']) }}" onclick="selectItem({{ $item['id'] }}, '{{ addslashes($item['name']) }}', {{ $item['contiti'] }})">
                  <div class="flex items-center gap-3">
                  ...
                  <span class="text-xs font-bold text-on-surface-variant bg-surface-container px-2.5 py-1 rounded-full">Stock: {{ $item['contiti'] }}</span>
                </div>
                @endforeach
              @endif
            </div>
          ...
          <div>
            ...
            <textarea rows="5" name="historique" placeholder="Order notes..." class="w-full rounded-xl border border-outline-variant bg-white px-4 py-3 text-sm resize-none focus:border-primary focus:ring-1 focus:ring-primary outline-none">{{ old('historique', '') }}</textarea>
          </div>
          ...
          <div>
            ...
            <label class="flex items-center gap-3 p-4 rounded-xl border border-outline-variant/20 bg-surface-container-low cursor-pointer">
              <input type="checkbox" name="is_urgent" value="1" class="accent-primary w-4 h-4">
              <div>
                <p class="font-bold text-sm text-on-surface">Urgent Order</p>
                <p class="text-xs text-on-surface-variant">Mark this order as urgent.</p>
              </div>
            </label>
          </div>
          ...
          <div class="flex gap-3 pt-2">
            <button type="button" onclick="showSection('overview')" class="flex-1 sm:flex-none px-6 py-3 rounded-xl border border-outline-variant text-on-surface-variant font-bold text-sm hover:bg-surface-container transition-colors">Cancel</button>
    ...
    <div id="section-orders" class="section-panel hidden space-y-6">
      ...
      <div class="fade-in fade-in-1 flex flex-wrap gap-2">
        <button class="filter-btn active-filter px-4 py-2 rounded-full text-xs font-bold border transition-colors" data-filter="all" onclick="filterOrders('all')">All ({{ $totalOrders ?? 0 }})</button>
        <button class="filter-btn px-4 py-2 rounded-full text-xs font-bold border transition-colors" data-filter="pending" onclick="filterOrders('pending')">Pending ({{ $pendingOrders ?? 0 }})</button>
        <button class="filter-btn px-4 py-2 rounded-full text-xs font-bold border transition-colors" data-filter="active" onclick="filterOrders('active')">In Progress ({{ $activeOrders ?? 0 }})</button>
        <button class="filter-btn px-4 py-2 rounded-full text-xs font-bold border transition-colors" data-filter="completed" onclick="filterOrders('completed')">Completed ({{ $completedOrders ?? 0 }})</button>
        <button class="filter-btn px-4 py-2 rounded-full text-xs font-bold border transition-colors" data-filter="cancelled" onclick="filterOrders('cancelled')">Cancelled ({{ $cancelledOrders ?? 0 }})</button>
      </div>
          ...
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-outline-variant/10">
              ...
              <tr class="order-row hover:bg-surface-container-low transition-colors" data-status="{{ $statusConfig['group'] }}">
                ...
                <td class="px-5 py-4 text-on-surface-variant text-xs">{{ isset($order['created_at']) ? date('d M Y', strtotime($order['created_at'])) : '—' }}</td>
                ...
                <td class="px-5 py-4 text-right">
                  <button onclick="openTrackingModal(@json($order))" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-white text-xs font-bold hover:bg-primary/90 active:scale-95 transition-all">
    ...
    <div id="section-tracking" class="section-panel hidden space-y-6">
      ...
      @php
        $activeDeliveries = array_filter($orders ?? [], fn($o) => in_array($o['status'] ?? '', ['ASSIGNED_TO_DELIVERY','PICKED_UP','IN_TRANSIT']));
      @endphp
      ...
      @else
        <div class="fade-in">
          <iframe src="{{ route('pharmacy.tracking', ['embed' => 1]) }}" style="width:100%;height:520px;border:0;border-radius:12px;" title="Live tracking"></iframe>
        </div>
        ...
        <div class="fade-in bg-surface-container-lowest rounded-2xl border border-outline-variant/15 shadow-sm p-6 space-y-5">
          <div class="flex items-center justify-between">
            ...
            <span class="text-xs text-on-surface-variant">{{ isset($order['created_at']) ? date('d M Y', strtotime($order['created_at'])) : '' }}</span>
          </div>
          ...
          <div class="flex items-center gap-0">
            ...
            <div class="flex flex-col items-center min-w-[60px]">
              <div class="step-dot w-7 h-7 rounded-full border-2 flex items-center justify-center mb-1.5 transition-all {{ $isDone ? 'bg-tertiary border-tertiary' : ($isActive ? 'bg-primary border-primary shadow-md' : 'bg-white border-outline-variant') }}">
...
<div id="trackingModal" class="modal-overlay" onclick="closeModalOutside(event,'trackingModal')">
  <div class="modal-box bg-surface-container-lowest w-full max-w-lg mx-4 rounded-2xl shadow-2xl border border-outline-variant/15 p-8 max-h-[90vh] overflow-y-auto">
    ...
    <button onclick="closeModal('trackingModal')" class="w-full mt-5 py-3 rounded-xl border border-outline-variant text-on-surface-variant font-bold text-sm hover:bg-surface-container transition-colors">Close</button>
  </div>
</div>

<script>
const initialSection = @json($initialSection ?? 'overview');
const selectedItems = {};
...
function filterOrders(filter) {
  document.querySelectorAll('.filter-btn').forEach(btn => btn.classList.toggle('active-filter', btn.dataset.filter === filter));
...
function updateQty(id, value) {
  if (!selectedItems[id]) return;
  selectedItems[id].qty = Math.max(1, parseInt(value) || 1);
  const input = document.getElementById('qty-hidden-' + id);
  if (input) input.value = selectedItems[id].qty;
}
...
function renderSelected() {
  ...
  chips.querySelectorAll('.selected-chip').forEach(el => el.remove());
  ...
  if (ids.length === 0) {
    ...
    ids.forEach(id => {
      ...
      div.className = 'selected-chip flex items-center gap-2 bg-primary/10 border border-primary/20 px-3 py-2 rounded-xl text-xs font-bold';
      div.innerHTML = `
  ...
  hidden.innerHTML = ids.map((id, index) => {
    const item = selectedItems[id];
    return `
      <input type="hidden" name="items[${index}][orderitem_id]" value="${id}" />
      <input type="hidden" name="items[${index}][contiti]" id="qty-hidden-${id}" value="${item.qty}" />`;
  }).join('');
}

function prepareOrderForm() {
  const ids = Object.keys(selectedItems);
  const errorEl = document.getElementById('no-product-error');
  if (ids.length === 0) {
...
function openTrackingModal(order) {
  ...
  const icons = {
    'PENDING_COMMERCIAL_REVIEW':'hourglass_top','COMMERCIALLY_CONFIRMED':'verified','READY_FOR_DISPATCH':'inventory_2','ASSIGNED_TO_DELIVERY':'assignment_ind','PICKED_UP':'shopping_bag','IN_TRANSIT':'local_shipping','DELIVERED':'done_all','COMPLETED':'check_circle','RETURNED':'assignment_return','CANCELLED':'cancel','CANCELLED_AFTER_RETURN':'cancel'
  };

  if (history.length === 0) {
    const status = order.status || '';
    timeline.innerHTML = `<div class="flex items-start gap-3 p-3 bg-surface-container-low rounded-xl">
      <span class="material-symbols-outlined text-primary mt-0.5" style="font-size:18px">${icons[status] || 'circle'}</span>
      <div><p class="font-bold text-sm text-on-surface">${escapeHtml(statusLabels[status] || status)}</p>
    ...
    timeline.innerHTML = history.map((h, i) => `
      <div class="flex items-start gap-3 ${i === 0 ? 'p-3 bg-primary/5 border border-primary/20 rounded-xl' : 'p-3 rounded-xl hover:bg-surface-container-low'}">
        ...
        <div class="flex-1">
          <p class="font-bold text-sm text-on-surface">${escapeHtml(h.to_status || '')}</p>
          <p class="text-xs text-on-surface-variant mt-0.5">${escapeHtml(h.note || '')}</p>
        </div>
  ...
  if (items.length > 0) {
    ...
    itemsList.innerHTML = items.map((it, idx) => `
      <div class="flex items-center justify-between px-4 py-3 bg-surface-container-low rounded-xl border border-outline-variant/15">
        <div class="flex items-center gap-3">
          <div class="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center text-primary text-xs font-bold flex-shrink-0">${idx + 1}</div>
          <span class="font-semibold text-sm text-on-surface">${escapeHtml(it.Name || it.name || '—')}</span>
        </div>
        <span class="bg-secondary-container text-on-secondary-container px-3 py-1 rounded-full text-xs font-bold">${escapeHtml(String(it.contiti || 0))} units</span>
      </div>`).join('');
  } else {
  ...
  paymentBadge.textContent = paymentStatus.replace('_',' ');
  paymentBadge.className = 'px-2.5 py-1 rounded-full text-xs font-bold ' + (statusColors[paymentStatus] || 'bg-surface-container text-on-surface-variant');
