@php
		...
		$statusLabelMap = [0 => 'En attente', 1 => 'Livré', 3 => 'Non livré'];
		$statusBgMap = [0 => 'bg-secondary-container text-on-secondary-container', 1 => 'bg-green-100 text-green-700', 3 => 'bg-error-container text-error'];
		$statusIconMap = [0 => 'pending', 1 => 'check_circle', 3 => 'cancel'];
		$dataStatusMap = [0 => 'attente', 1 => 'livre', 3 => 'nonlivre'];
...
<script>
	...
	window.DP_REQUIRE_GPS_ON_LOAD = true;
</script>
...
<style>
	...
	body.gps-locked #delivery-dashboard-shell{filter:blur(3px);pointer-events:none;user-select:none;}
	body.gps-locked #delivery-gps-lock-banner{display:flex;}
...
<div id="delivery-gps-lock-banner" class="hidden fixed inset-0 z-[80] items-center justify-center px-4 bg-black/35 backdrop-blur-[2px]">
	<div class="max-w-md w-full bg-white rounded-3xl shadow-2xl border border-outline-variant/15 p-6 text-center">
		<div class="w-14 h-14 mx-auto rounded-2xl bg-error-container text-error flex items-center justify-center mb-4">
			<span class="material-symbols-outlined" style="font-variation-settings:'FILL' 1;">gps_fixed</span>
		</div>
		<h2 class="font-headline text-2xl font-extrabold text-on-surface">GPS required</h2>
		<p class="mt-2 text-sm text-on-surface-variant leading-relaxed">
			Enable GPS first. The dashboard stays locked until your location is active.
		</p>
	</div>
</div>
...
<div id="delivery-dashboard-shell">
	...
	<div class="fade-in space-y-4" id="orders-container" style="animation-delay:.15s">
		...
		@else
			@foreach ($orders as $i => $order)
				...
				<div class="order-card bg-surface-container-lowest rounded-2xl border {{ $urg ? 'border-error/40 urgent-ring' : 'border-outline-variant/15' }} shadow-sm overflow-hidden" data-status="{{ $dataStatusMap[$st] ?? 'attente' }}" style="animation:fadeUp .35s ease {{ $i * .06 }}s both;">
					...
					<div class="p-5 space-y-4">
						<div class="flex flex-wrap items-center gap-2">
							...
							<span class="inline-flex items-center gap-1 text-[10px] font-black px-2.5 py-0.5 rounded-full uppercase {{ $statusBgMap[$st] ?? 'bg-surface-container text-on-surface-variant' }}">
								<span class="material-symbols-outlined text-xs" style="font-variation-settings:'FILL' 1;">{{ $statusIconMap[$st] ?? 'help' }}</span>
								{{ $statusLabelMap[$st] ?? 'Inconnu' }}
							</span>
							@if ($urg)
						...
						@if (!empty($order['proof']))
							<div class="flex items-center gap-3">
								<img src="{{ asset($order['proof']) }}" alt="Preuve"
									...
									onclick="window.open(@js(asset($order['proof'])), '_blank')"/>
								<div>
						...
						<div class="flex gap-2 flex-wrap pt-1">
							@if ($st === 0)
								<button onclick="markTransit(@json($track), this)" class="flex items-center justify-center gap-2 bg-blue-500 text-white px-4 py-3 rounded-xl font-bold text-sm hover:opacity-90 active:scale-95 transition-all shadow-sm" id="btn-transit-{{ $track }}">
									<span class="material-symbols-outlined text-lg" style="font-variation-settings:'FILL' 1;">local_shipping</span>
									Chargé en transit
								</button>
								<button onclick="openCameraModal(@json($track), 'livre')" class="flex-1 flex items-center justify-center gap-2 bg-green-600 text-white px-4 py-3 rounded-xl font-bold text-sm hover:opacity-90 active:scale-95 transition-all shadow-sm {{ $urg ? 'ring-2 ring-error/50' : '' }}">
									<span class="material-symbols-outlined text-lg" style="font-variation-settings:'FILL' 1;">check_circle</span>
									Livré
								</button>
								<button onclick="confirmAction(@json($track), 'non_livre', @json('Marquer #' . $track . ' comme non livré ?'))" class="flex items-center justify-center gap-2 bg-error text-white px-4 py-3 rounded-xl font-bold text-sm hover:opacity-90 active:scale-95 transition-all shadow-sm">
	...
	<div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
		...
		<div class="p-5 space-y-3">
			<div id="captureActions">
				<button onclick="capturePhoto()" class="w-full flex items-center justify-center gap-2 py-3 bg-primary text-white font-bold rounded-xl hover:opacity-90 active:scale-95 transition-all">
			...
			<div id="confirmActions" class="hidden space-y-3">
				...
				<div class="flex gap-2">
					...
					<button onclick="submitCameraAction()" class="flex-1 py-2.5 bg-primary text-white rounded-xl text-sm font-bold hover:opacity-90 active:scale-95 transition-all flex items-center justify-center gap-1">
...
<div id="confirmModal" class="modal-bg">
	<div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-7 text-center">
		...
		<div class="flex gap-3">
			...
			<button id="confirmBtn" onclick="submitConfirm()" class="flex-1 py-3 rounded-xl font-bold text-sm text-white active:scale-95 transition-all">
				Confirmer
			</button>
		</div>
	</div>
